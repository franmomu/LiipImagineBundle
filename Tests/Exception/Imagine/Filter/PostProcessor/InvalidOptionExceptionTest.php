...
class InvalidOptionExceptionTest extends TestCase
{
    ...
    public static function provideExceptionMessageData()
    {
        return [
            ...
            ['a foobar message', ['baz' => new \stdClass()], 'baz="(object) array()"'],
            ['a foobar message', ['foo' => 'bar', 'baz' => new \stdClass()], 'foo="bar", baz="(object) array()"'],
            ['a foobar message', ['foo' => 'bar', 'baz' => new \stdClass(), 'int' => 100, 'array' => ['this', 'that']], 'foo="bar", baz="(object) array()", int="100", array="["this","that"]"'],
            ['a foobar message', ['foo' => 'bar', 'baz' => new \stdClass(), 'int' => 100, 'array' => ['this' => 'that']], 'foo="bar", baz="(object) array()", int="100", array="{"this":"that"}"'],
        ];
    }
